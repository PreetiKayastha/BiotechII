...
#load result
resultmaizedia <- read.csv("GAPIT.Association.GWAS_Results.MLM.EarDia.csv",  header = T)
head(resultmaizedia)


##Activity
#manhatten plot
manhattan(resultmaizedia, main="Ear Diameter", chr = "Chr", bp = "Pos", p = "P.value", snp = "SNP",           
          col = c("cyan", "pink"),           
...
##Interesting part 1: using multiple models
# Load GAPIT
...
myGD <- read.table("http://zzlab.net/GAPIT/data/mdp_numeric.txt", head = TRUE)
myGM <- read.table("http://zzlab.net/GAPIT/data/mdp_SNP_information.txt", head = TRUE)

# Run GWAS with multiple models
myGAPIT <- GAPIT(
  Y = myY[, c(1, 2)],
  GD = myGD,
  GM = myGM,
